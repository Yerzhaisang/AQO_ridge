...
for filename in onlyfiles:
    ...
    if os.path.isfile('/home/yerzh/aqo/data.csv'):
        ...
        df = pd.DataFrame(columns=['AQO(KNN)','Actual'])
    plan_rows = []
    actual_rows = []
    for j in dictt[filename]['nodes']:
        plan_rows.append(j[0])
        actual_rows.append(j[1])
    df = df.append(pd.DataFrame(np.array([plan_rows, actual_rows]).T, columns=['AQO(KNN)','Actual'], index=[filename for t in plan_rows]))
